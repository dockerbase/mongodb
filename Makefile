...
run:
	docker run -p 27017:27017 -v /data/db:/data/db --name $(subst /,-,$(NAME)) --restart=always -t --cidfile cidfile -d $(NAME):$(VERSION) /sbin/runit
...
version:
	...
	docker run -it --rm $(NAME):$(VERSION) sh -c " mongod --version ; mongo --version " | tee -a COMPONENTS
	dos2unix COMPONENTS
